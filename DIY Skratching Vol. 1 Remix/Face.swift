...
class Face: UIViewController {
    ...
    override func viewWillLayoutSubviews() {
        ...
        case .portrait,.portraitUpsideDown,.faceDown,.faceUp,.unknown:
            layerRect = CGRect(x: self.videoView.frame.origin.x-540.0, y: self.videoView.frame.origin.y, width: self.videoView.frame.height*960.0/540, height: self.videoView.frame.height)
            break
